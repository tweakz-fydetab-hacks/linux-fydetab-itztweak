...
The `linux-fydetab` package may depend on specific kernel headers or module versions. Updating system packages before rebuilding the kernel can cause mismatches. Following this procedure ensures compatibility.
...
If paru tries to replace `linux-fydetab` with a different kernel, decline or exclude it:
```sh
...
1. Check the build log: `~/builds/pkgbuilds/linux-fydetab/logs/build-latest.log`
2. Common fixes:
...
- Build logs are saved to `~/builds/pkgbuilds/linux-fydetab/logs/`
